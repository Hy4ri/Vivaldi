(async () => {
    ...
    const getBaseDomain = (url) => {
        ...
        return (
            hostForSecurityDisplay.match(`([^.]+\\.${tld})$`)?.[1] ||
    ...
    const matchHostRule = (url, rule) => {
        ...
        return rule instanceof RegExp
            ? rule.test(hostForSecurityDisplay)
            : hostForSecurityDisplay === rule;
    };
    ...
    const getWorkspaceName = async (workspaceId) => {
        ...
        const workspaceList = await vivaldi.prefs.get(
        ...
        const tabs = (await chrome.tabs.query({ currentWindow: true }))
            ...
            .filter(
                (tab) =>
                    ...
                    config.allow_domains.find((rule) =>
                        matchHostRule(tab.url, rule),
                    ),
            )
            .filter(
                (tab) =>
                    !config.block_domains.length ||
                    !config.block_domains.find((rule) =>
                        matchHostRule(tab.url, rule),
                    ),
            );
    ...
    const getTabsByStack = (tabs) =>
        Object.groupBy(tabs, (tab) => tab.vivExtData.group);

    const getTabsByHost = (tabs) =>
    ...
    const getMaxTabsStackId = (tabsByStack, targetHost) => {
        ...
        for (const [stackId, tabs] of Object.entries(tabsByStack)) {
            if (stackId !== "undefined") {
                ...
                counts[stackId] = Object.values(tabsByHost).reduce(
                    (acc, tabs) => {
    ...
    const stackingTabs = async (workspaceId) => {
        ...
        if (
            !config.allow_workspaces.length ||
            config.allow_workspaces.includes(workspaceName)
        ) {
            ...
            for (const [host, tabs] of Object.entries(tabsByHost)) {
                const targetStackId =
                    getMaxTabsStackId(tabsByStack, host) || crypto.randomUUID();
                const targetStackTabs = tabsByStack[targetStackId]
                    ? getTabsByHost(tabsByStack[targetStackId])[host]
                    : [];
                const targetTabs = mergeArrays(targetStackTabs, tabs);
                const targetStackName = getTabStackName(
